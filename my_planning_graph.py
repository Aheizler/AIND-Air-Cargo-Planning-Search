...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Create an empty set in the level to store actions
        a_nodes = set()
        for action in self.all_actions:
            # Build an action Node for this action
            node_a = PgNode_a(action)
            # The action Node's preconditions must be satisfied by the previous state level in order to be accessible
            if node_a.prenodes.issubset(self.s_levels[level]):
                # Add this accessible action node to the set of action nodes
                a_nodes.add(node_a)
                for node_s in self.s_levels[level]:
                    # Make this accessible action node a child of the preceding level's state nodes
                    node_s.children.add(node_a)
                    # Make the preceding level's state nodes parents of this accessible action node
    ...
    def add_literal_level(self, level):
        ...
        # Create an empty set in the level to store S literal nodes
        s_nodes = set()
        for node_a in self.a_levels[level - 1]:
            # Get all the effect nodes for this preceding action and add them to the level's state nodes
            for node_s in node_a.effnodes:
                # Add this accessible effect node to the set of state nodes
                s_nodes.add(node_s)
                # Make the preceding level's action node a parent of this accessible effect node
                node_s.parents.add(node_a)
                # Make this accessible effect node a child of the preceding action node
                node_a.children.add(node_s)
                
        # Create the state level from the accessible effect nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Determine if node_a1's pos effects are not negated by node_a2's rem effects
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True
            
        # Determine if node_a2's pos effects are not negated by node_a1's rem effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Determine if node_a1's pos effects do not conflict with node_a2's neg preconditions
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True
        
        # Determine if node_a2's pos effects do not conflict with node_a1's neg preconditions
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True
            
        # Determine if node_a1's rem effects do not conflict with node_a2's pos preconditions
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True
            
        # Determine if node_a2's rem effeccts do not conflict with node_a1's pos preconditions
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Determine if node_a1's preconditions aren't mutually exclusive with node_a2's preconditions
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # Determine if one state literal negates another state literal
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # Determine if node_s1's preconditions are not mutually exclusive with node_s2's preconditions
